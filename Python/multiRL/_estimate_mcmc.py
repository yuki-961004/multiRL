"""Internal MCMC estimator for the Python multiRL frontend."""

from . import _backend as _cpp_estimate_mcmc
...
def estimate_mcmc(
    ...
    cpp_result = _cpp_estimate_mcmc.estimate_mcmc(
        ...
        warmup=int(control.get("warmup", 1000)),
        samples=int(control.get("samples", 1000)),
        chains=int(control.get("chains", 4)),
        thin=int(control.get("thin", 1)),
        step_size=float(control.get("step_size", 0.1)),
        target_accept=float(control.get("target_accept", 0.8)),
        max_tree_depth=int(control.get("max_tree_depth", 10)),
        seed=int(control.get("seed", 1004)),
        algorithm=str(control.get("algorithm", "nuts")),
