---
title: "Step 1: run_m"
author: "YuKi"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Step 1: run_m}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---
<p align="center">
    <img src="./fig/rl_step1.png" alt="Step 1" width="80%" style="display: inline;">
</p>

```{r}
data <- multiRL::TAB
behrule = list(
  cue = c("A", "B", "C", "D"),
  rsp = c("A", "B", "C", "D")
)
colnames = list(
  subid = "Subject", block = "Block", trial = "Trial",
  object = c("L_choice", "R_choice"), 
  reward = c("L_reward", "R_reward"),
  action = "Sub_Choose",
  exinfo = c("Frame", "NetWorth", "RT")
)
```

## TD

<p align="center">
    <img src="./fig/TD.png" alt="TD" width="80%" style="display: inline;">
</p>

```{r}
multiRL.model <- multiRL::run_m(
  #engine = "R",
  data = data[data[, "Subject"] == 1, ],
  behrule = behrule,
  colnames = colnames,
  params = list(
    free = list(
      alpha = 0.5,
      beta = 0.5
    )
  ),
  priors = list(
    alpha = function(x) {stats::dbeta(x, shape1 = 2, shape2 = 2, log = TRUE)}, 
    beta = function(x) {stats::dexp(x, rate = 1, log = TRUE)}
  ),
  settings = list(name = "TD", policy = "on")
)

multiRL.summary <- multiRL::summary(multiRL.model)
```

## RSTD

<p align="center">
    <img src="./fig/RSTD.png" alt="RSTD" width="80%" style="display: inline;">
</p>

```{r}
multiRL.model <- multiRL::run_m(
  data = data[data[, "Subject"] == 1, ],
  behrule = behrule,
  colnames = colnames,
  params = list(
    free = list(
      alphaN = 0.3,
      alphaP = 0.7,
      beta = 0.5
    )
  ),
  priors = list(
    alphaN = function(x) {stats::dbeta(x, shape1 = 2, shape2 = 2, log = TRUE)}, 
    alphaP = function(x) {stats::dbeta(x, shape1 = 2, shape2 = 2, log = TRUE)}, 
    beta = function(x) {stats::dexp(x, rate = 1, log = TRUE)}
  ),
  settings = list(name = "RSTD", policy = "on")
)

multiRL.summary <- multiRL::summary(multiRL.model)
```

## Utility

<p align="center">
    <img src="./fig/Utility.png" alt="Utility" width="80%" style="display: inline;">
</p>

```{r}
multiRL.model <- multiRL::run_m(
  data = data[data[, "Subject"] == 1, ],
  behrule = behrule,
  colnames = colnames,
  params = list(
    free = list(
      alpha = 0.5,
      beta = 0.5,
      gamma = 0.5
    )
  ),
  priors = list(
    alpha = function(x) {stats::dbeta(x, shape1 = 2, shape2 = 2, log = TRUE)}, 
    beta = function(x) {stats::dexp(x, rate = 1, log = TRUE)},
    gamma = function(x) {stats::dbeta(x, shape1 = 2, shape2 = 2, log = TRUE)}
  ),
  settings = list(name = "Utility", policy = "on")
)

multiRL.summary <- multiRL::summary(multiRL.model)
```

## Prospect

<p align="center">
    <img src="./fig/Prospect.png" alt="Prospect" width="80%" style="display: inline;">
</p>

Below, I will implement a new `func_gamma` to ensure compatibility with both Stevens' Power Law and Prospect Theory.

```{r}
my_func_gamma <- function(
    shown,
    reward,
    rownum,
    params,
    hidden,
    ...
){
  # `get_param` is an internal function and must be accessed using `:::`
  lambda    <-  params[["lambda"]]
  gamma     <-  params[["gamma"]]
  gammaN    <-  params[["gammaN"]]
  gammaP    <-  params[["gammaP"]]
  
  if (
    gamma != 1 && is.na(gammaN) && is.na(gammaP)
  ) {
    model <- "Utility"
  } else if (
    gamma == 1 && !(is.na(gammaN)) && !(is.na(gammaP))
  ) {
    model <- "Prospect"
  } else {
    stop("Unknown Model! Plase modify your utility function")
  }
  
  # Stevens's Power Law
  if (model == "Utility") {
    utility <- sign(reward) * (abs(reward) ^ gamma)
  }
  # Prospect Theory
  else if (model == "Prospect" && reward < 0) {
    utility <- lambda * sign(reward) * (abs(reward) ^ gammaN)
  }
  else if (model == "Prospect" && reward >= 0) {
    utility <- sign(reward) * (abs(reward) ^ gammaP)
  }
  
  return(list(output = utility, hidden = hidden))
}
```

```{r}
multiRL.model <- multiRL::run_m(
  data = data[data[, "Subject"] == 1, ],
  behrule = behrule,
  colnames = colnames,
  params = list(
    free = list(
      alpha = 0.5,
      beta = 0.5,
      lambda = 1.5,
      gammaN = 0.7,
      gammaP = 0.3
    )
  ),
  priors = list(
    alpha = function(x) {stats::dbeta(x, shape1 = 2, shape2 = 2, log = TRUE)}, 
    beta = function(x) {stats::dexp(x, rate = 1, log = TRUE)},
    lambda = function(x) {stats::dnorm(x, mean = 2, sd = 0.3, log = TRUE)},
    gammaN = function(x) {stats::dbeta(x, shape1 = 2, shape2 = 2, log = TRUE)},
    gammaP = function(x) {stats::dbeta(x, shape1 = 2, shape2 = 2, log = TRUE)}
  ),
  funcs = list(
    # Other unmodified `funcs` will use the built-in functions.
    util_func = my_func_gamma
  ),
  settings = list(name = "Prospect", policy = "on")
)

multiRL.summary <- multiRL::summary(multiRL.model)
```

## RLWM

<p align="center">
    <img src="./fig/WM.png" alt="WM" width="50%" style="display: inline;">
</p>

For more information, see https://github.com/yuki-961004/RLWMH

```{r}
data <- multiRL::WMT
behrule = list(
  cue = c(
    "AJ", "AK", "AL", 
    "BJ", "BK", "BL", 
    "CJ", "CK", "CL", 
    "DJ", "DK", "DL", 
    "EJ", "EK", "EL", 
    "FJ", "FK", "FL"
  ),
  rsp = c(
    "AJ", "AK", "AL", 
    "BJ", "BK", "BL", 
    "CJ", "CK", "CL", 
    "DJ", "DK", "DL", 
    "EJ", "EK", "EL", 
    "FJ", "FK", "FL"
  )
)
colnames = list(
  subid = "Subject", block = "Block", trial = "Trial",
  object = c("Object_1", "Object_2", "Object_3"),
  reward = c("Reward_1", "Reward_2", "Reward_3"),
  action = "Action",
  exinfo = c("SetSize")
)
```

```{r}
multiRL.model <- multiRL::run_m(
  data = data[data[, "Subject"] == 1, ],
  behrule = behrule,
  colnames = colnames,
  params = list(
    free = list(
      alpha = 0.5,
      beta = 25,
      zeta = 0.5,
      weight = 0.5
    )
  ),
  priors = list(
    alpha = function(x) {stats::dbeta(x, shape1 = 2, shape2 = 2, log = TRUE)}, 
    beta = function(x) {stats::dexp(x, rate = 0.1, log = TRUE)},
    zeta = function(x) {stats::dbeta(x, shape1 = 2, shape2 = 2, log = TRUE)}, 
    weight = function(x) {stats::dbeta(x, shape1 = 2, shape2 = 2, log = TRUE)}
  ),
  settings = list(name = "RLWM", policy = "on", system = c("RL", "WM"))
)

multiRL.summary <- multiRL::summary(multiRL.model)
```

