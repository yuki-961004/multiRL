...
inline bool criterion_same_string_vector(
...
template <typename T>
inline CriterionValue<T> criterion_likelihood_value(
    const RunTask& task,
    ...
    if (!criterion_same_string_vector(task.behrule.cue, task.behrule.rsp)) {
        return metric;
    }
