...
def test_rpl_e_on_fit(data_sub):
    fit = multiRL.fit_p(
        estimator="mle",
        data=data_sub,
        colnames={
    ...
    rep = multiRL.rpl_e(fit, option={"plot": False})
    assert rep is not None
    assert "replay" in rep
    assert "plot_data" in rep

def test_rpl_e_on_rcv_d(data_sub):
    rec = multiRL.rcv_d(
        estimator="mle",
        data=data_sub,
        colnames={
    ...
    rep = multiRL.rpl_e(rec, option={"plot": False})
    assert rep is not None
    assert "recovery" in rep
    assert "model_recovery" in rep
