...
template <typename T>
CriterionValue<T> criterion_compute(
    const RunTask& task,
    ...
    if (!criterion_equal_strings(task.behrule.cue, task.behrule.rsp)) {
        return metric;
    }
    ...
    // 12.0 is a sentinel value for elastic-net regularization
    // (L1 + L2 combined). Any other value > 0 uses the
    // corresponding L-norm bridge penalty.
    const double L_NORM_ELASTIC_NET = 12.0;
    if (!std::isnan(l_norm)) {
        T regularizer = 0.0;
        for (const std::string& name : task.params.free_names) {
            const T value =
                static_cast<T>(task.params.get(name));
            if (l_norm == L_NORM_ELASTIC_NET) {
                regularizer +=
...
public:
    ...
    CriterionValue<T> evaluate(
        ...
        return criterion_compute<T>(task, output);
    }
